...
struct YPaintGlyph {
  ...
  float width() const {
    ...
    uint32_t f32bits;
    if (exp == 0)
      f32bits = sign; // zero/denorm->0
    else if (exp == 31)
      f32bits = sign | 0x7F800000; // inf
    else
      f32bits = sign | ((exp + 112) << 23) | (mant << 13);
    float r;
    std::memcpy(&r, &f32bits, 4);
    return r;
  }
  float height() const {
...
//=============================================================================
// Painter - AABB computation and spatial grid builder for scrolling SDF
//
// Builds a spatial hash grid for GPU culling from YPaintBuffer primitives.
// Uses a two-level architecture:
//   Level 1: Internal incremental structure
//   (std::vector<std::vector<uint32_t>>) Level 2: Packed GPU format (cache,
//   rebuilt when dirty)
//
// Usage:
//   auto builder = Painter::create(fontMgr, alloc, cardMgr, slot);
//   builder->addYpaintBuffer(buffer1);  // Incrementally adds primitives
//   builder->addYpaintBuffer(buffer2);  // Can add multiple buffers
//=============================================================================
class Painter : public core::FactoryObject<Painter> {
public:
  ...
  // Build sorted reading order (by Y then X). Call once after content is
  // finalized.
  ...
  // Set selection range (sorted-order indices). Sets GLYPH_FLAG_SELECTED on
  // glyphs. Pass (-1, -1) to clear.
  ...
  addYpaintBuffer(std::shared_ptr<YPaintBuffer> buffer) = 0;
