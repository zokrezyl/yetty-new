...
private:
  ...
  template <typename FType, typename... Args> struct HasCreateImplWithContext {
  private:
    template <typename F>
    static auto check(F *)
        -> decltype(F::Type::createImpl(
                        std::declval<typename F::ContextType &>(),
                        std::declval<Args>()...),
                    std::true_type{});
  ...
  template <typename FType> struct HasCreateImplWithContextNoArgs {
  private:
    template <typename F>
    static auto check(F *)
        -> decltype(F::Type::createImpl(
  ...
  template <typename FType, typename... Args> struct HasCreateImpl {
  private:
    template <typename F>
    static auto check(F *)
        -> decltype(F::Type::createImpl(std::declval<Args>()...),
                    std::true_type{});
    template <typename> static std::false_type check(...);
  ...
  template <typename FType> struct HasCreateImplNoArgs {
  private:
    template <typename F>
...
//
template <typename T, typename ContextT = SingletonContext> class Singleton {
public:
  using ContextType = ContextT;
  using Type = T;
  using Ptr = std::shared_ptr<T>;
...
private:
  template <typename SType> struct HasCreateImplWithContext {
  private:
    template <typename S>
    static auto check(S *)
        -> decltype(S::Type::createImpl(
                        std::declval<typename S::ContextType &>()),
                    std::true_type{});
    template <typename> static std::false_type check(...);
  ...
  template <typename SType> struct HasCreateImplNoArgs {
  private:
    template <typename S>
