...
/**
 * CardBufferManager - CPU-side linear buffer sub-allocator.
 *
 * Cards/painters request sub-allocations, write data directly to returned pointers.
 * GpuMemoryManager aggregates this into GpuResourceSet for GPU upload.
 */
class CardBufferManager : public core::FactoryObject<CardBufferManager> {
public:

    static Result<CardBufferManager*> createImpl();
    ...
    virtual const uint8_t* bufferData() const = 0;
