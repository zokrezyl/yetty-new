// local_id 0x0028 — signal-bars

fn signalBars_hash(n: f32) -> f32 {
    return fract(sin(n * 127.1) * 43758.5453);
}

fn shader_glyph_40(local_uv: vec2<f32>, time: f32, fg: vec3<f32>, bg: vec3<f32>, pixel_pos: vec2<f32>) -> vec3<f32> {
    let numBars = 4.0;
    let barGap = 0.04;
    let totalW = 0.7;
    ...
    for (var i = 0; i < 4; i++) {
        ...
        let inX = step(barX, local_uv.x) * step(local_uv.x, barX + barW);
        let inY = step(baseY - h, local_uv.y) * step(local_uv.y, baseY);
