// local_id 0x00f5 — butterfly-flock
...
fn shader_glyph_245(local_uv: vec2<f32>, time: f32, fg: vec3<f32>, bg: vec3<f32>, pixel_pos: vec2<f32>) -> vec3<f32> {
    ...
    var uv = pixel_pos / screenSize - vec2<f32>(0.5);
    uv.y /= screenSize.x / screenSize.y;

    var mouse = ((uniforms.shader_glyph_grid_size * uniforms.shader_glyph_cell_size) * 0.5) / screenSize - vec2<f32>(0.5);
