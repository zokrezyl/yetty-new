// local_id 0x00fd — mandelbrot

fn shader_glyph_253(local_uv: vec2<f32>, time: f32, fg: vec3<f32>, bg: vec3<f32>, pixel_pos: vec2<f32>) -> vec3<f32> {
    ...
    let screenUV = pixel_pos / screenSize;
    let mouseUV = clamp(((uniforms.shader_glyph_grid_size * uniforms.shader_glyph_cell_size) * 0.5) / screenSize, vec2<f32>(0.0), vec2<f32>(1.0));
