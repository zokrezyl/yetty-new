// local_id 0x0009 — rocket

fn shader_glyph_9_hash(p: vec2<f32>) -> f32 {
    return fract(sin(dot(p, vec2<f32>(127.1, 311.7))) * 43758.5453);
}

fn shader_glyph_9(local_uv: vec2<f32>, time: f32, fg: vec3<f32>, bg: vec3<f32>, pixel_pos: vec2<f32>) -> vec3<f32> {
    // fg unused - rocket has its own colors
    ...
    if (flameY > 0.0 && flameY < 0.5 && abs(rp.x) < 0.15) {
        let flicker = shader_glyph_9_hash(vec2<f32>(time * 10.0, rp.x * 100.0));
