// local_id 0x001c — dna-helix

fn shader_glyph_28(local_uv: vec2<f32>, time: f32, fg: vec3<f32>, bg: vec3<f32>, pixel_pos: vec2<f32>) -> vec3<f32> {
    let cx = 0.5;
    let amplitude = 0.25;
    let freq = 6.28318;
    let scrollSpeed = 2.0;
    ...
    let y = local_uv.y + time * scrollSpeed * 0.1;
    ...
    // Strand lines
    let d1 = abs(local_uv.x - strand1_x);
    let d2 = abs(local_uv.x - strand2_x);
    ...
    let rungX = min(local_uv.x, 1.0 - local_uv.x);
    let withinStrands = step(min(strand1_x, strand2_x), local_uv.x) *
                        step(local_uv.x, max(strand1_x, strand2_x));
    ...
    for (var i = -3; i <= 3; i++) {
        ...
        let dd = length(local_uv - crossPos);
