// local_id 0x0021 — rain

fn rain_hash(n: f32) -> f32 {
    return fract(sin(n * 127.1) * 43758.5453);
}

fn shader_glyph_33(local_uv: vec2<f32>, time: f32, fg: vec3<f32>, bg: vec3<f32>, pixel_pos: vec2<f32>) -> vec3<f32> {
    let numDrops = 8;
    let dotRadius = 0.04;
    ...
    for (var i = 0; i < 8; i++) {
        ...
        let dy = abs(local_uv.y - y);
        let dx = abs(local_uv.x - x);
