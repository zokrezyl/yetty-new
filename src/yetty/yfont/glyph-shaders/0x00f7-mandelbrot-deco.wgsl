// local_id 0x00f7 — mandelbrot-deco
...
fn shader_glyph_247(local_uv: vec2<f32>, time: f32, fg: vec3<f32>, bg: vec3<f32>, pixel_pos: vec2<f32>) -> vec3<f32> {
    ...
    for (var j = 0; j < 2; j++) {
        for (var i = 0; i < 2; i++) {
            ...
            var p = (pixel_pos + vec2<f32>(f32(i), f32(j)) * 0.5 - screenSize * 0.5) / screenSize.y;
