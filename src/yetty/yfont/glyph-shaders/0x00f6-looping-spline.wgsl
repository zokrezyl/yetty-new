// local_id 0x00f6 — looping-spline
...
fn shader_glyph_246(local_uv: vec2<f32>, time: f32, fg: vec3<f32>, bg: vec3<f32>, pixel_pos: vec2<f32>) -> vec3<f32> {
    let screenSize = vec2<f32>((uniforms.shader_glyph_grid_size.x * uniforms.shader_glyph_cell_size.x), (uniforms.shader_glyph_grid_size.y * uniforms.shader_glyph_cell_size.y));
    ...
    if (((uniforms.shader_glyph_grid_size * uniforms.shader_glyph_cell_size) * 0.5).x > 0.0 && ((uniforms.shader_glyph_grid_size * uniforms.shader_glyph_cell_size) * 0.5).y > 0.0) {
        ctrlPts.p[2] = ((uniforms.shader_glyph_grid_size * uniforms.shader_glyph_cell_size) * 0.5) / screenSize.xx;
    }

    let uv = pixel_pos / screenSize.xx;
