// local_id 0x001b — hourglass

fn hourglass_hash(p: vec2<f32>) -> f32 {
    return fract(sin(dot(p, vec2<f32>(127.1, 311.7))) * 43758.5453);
}

fn hourglass_draw(uv_in: vec2<f32>, progress: f32, fg: vec3<f32>, bg: vec3<f32>) -> vec3<f32> {
    let uv = uv_in;
    let cx = 0.5;
    let waistY = 0.5;
    let topWidth = 0.35;
...
fn shader_glyph_27(local_uv: vec2<f32>, time: f32, fg: vec3<f32>, bg: vec3<f32>, pixel_pos: vec2<f32>) -> vec3<f32> {
    let sandTime = 3.0;
    let flipTime = 0.8;
    ...
    if (inFlip) {
        ...
        return hourglass_draw(rotated, frozenProgress, fg, bg);
    }
    ...
    return hourglass_draw(local_uv, progress, fg, bg);
}
