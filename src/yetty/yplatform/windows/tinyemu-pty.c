...
#ifdef _WIN32
    /* Windows select() only works with Winsock SOCKETs, not CRT pipe fds.
     * Sleep for the timer delay; poll input non-blockingly below. */
    Sleep((DWORD)delay);
    ret = m->console_dev && virtio_console_can_write_data(m->console_dev) ? 1 : 0;
#else
    ret = select(fd_max + 1, &rfds, &wfds, &efds, &tv);
#endif

    /* Feed keyboard input to VM. read() on a Win32 _pipe is BLOCKING by
     * default; PeekNamedPipe first to skip the read when there's no data. */
    if (ret > 0 && m->console_dev) {
        int has_data = 1;
#ifdef _WIN32
        DWORD avail = 0;
        HANDLE h = (HANDLE)_get_osfhandle(pty->os_input_pipe[0]);
...
#else
        if (!FD_ISSET(pty->os_input_pipe[0], &rfds)) has_data = 0;
#endif
        if (has_data) {
            uint8_t buf[128];
...
static int init_vm(struct tinyemu_pty *pty)
{
    ...
    /* Drop eth entries whose backend was never created (otherwise
     * virtio_net_init deref's a NULL EthernetDevice). */
    {
        int kept = 0;
        for (int i = 0; i < p->eth_count; i++) {
            if (p->tab_eth[i].net) {
                if (kept != i) p->tab_eth[kept] = p->tab_eth[i];
                kept++;
            }
        }
        p->eth_count = kept;
...
struct yetty_yplatform_pty_result tinyemu_pty_create(struct yetty_yconfig *config)
{
     ...
     * absolute paths so tinyemu's get_file_path() resolves them as-is. */
    {
        ...
        if (cfg_ready && access(cfg_path, F_OK) != 0) {
            ...
            if (f) {
                /* Normalize path separators for JSON. Backslashes are JSON
                 * escape characters; on Windows data_dir is "C:\Users\...".
                 * Forward slashes are valid path separators in the C runtime
                 * and don't need escaping. */
                char data_dir_fwd[512];
                snprintf(data_dir_fwd, sizeof(data_dir_fwd), "%s", data_dir);
                for (char *p = data_dir_fwd; *p; p++) {
                    if (*p == '\\') *p = '/';
                }
                fprintf(f,
                    ...
                    "    cmdline: \"earlycon=sbi console=hvc0 root=/dev/vda rw init=/bin/sh\",\n"
                    "    drive0: { file: \"%s/yemu/alpine-rootfs.img\" },\n"
                    "    eth0: { driver: \"user\" }\n"
                    "}\n",
                    data_dir_fwd, data_dir_fwd, data_dir_fwd);
                fclose(f);
...
struct yetty_yplatform_pty_factory_result tinyemu_pty_factory_create(
    struct yetty_yconfig *config)
{
