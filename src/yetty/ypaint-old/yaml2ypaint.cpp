...
static uint32_t parseColor(const YAML::Node& node) {
    ...
    if (str[0] == '#') {
        ...
        try {
            ...
            ywarn("yaml2ypaint: failed to parse color value '{}'", str);
            return 0xFFFFFFFF;
        }
    }
    return 0xFFFFFFFF;
...
static Result<void> parseYAMLPrimitive(YPaintBuffer* buffer,
    ...
    if (item["text"]) {
        ...
        if (t["font"]) {
            ...
            if (!f) {
                // Warn but continue - don't abort entire YAML parsing for missing font
                ywarn("yaml2ydraw: font not found: {} - text will use default font", ttfPath);
            } else {
                auto sz = f.tellg();
                f.seekg(0);
