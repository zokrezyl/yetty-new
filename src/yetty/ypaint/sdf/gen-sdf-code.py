...
SDF_YAML_H_OUT = PROJECT_ROOT / "include" / "yetty" / "ypaint" / "yaml" / "ypaint-yaml.gen.h"
SDF_YAML_C_OUT = PROJECT_ROOT / "src" / "yetty" / "ypaint" / "yaml" / "ypaint-yaml.gen.c"
...
def generate_sdf_yaml_impl(prims: list[dict], out: Path) -> None:
    ...
    lines = [HEADER_C, ""]
    lines.append('#include <yetty/ypaint/yaml/ypaint-yaml.gen.h>')
    lines.append('#include <yetty/ypaint/sdf/ypaint-sdf-types.gen.h>')
    lines.append('#include <yetty/ypaint/core/ypaint-buffer.h>')
    ...
    lines.append("    if (!err && yaml_parse_ctx.buffer->prims.buf.size > 0) {")
