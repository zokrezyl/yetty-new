...
struct ypaint_canvas_grid_line {
	struct ypaint_canvas_prim_data_array prims;  // primitives whose BASE is this line
				     ...
				     const float *data, uint32_t word_count)
{
	if (arr->count >= arr->capacity) {
		...
		arr->data = realloc(arr->data, new_cap * sizeof(struct ypaint_canvas_prim_data));
		arr->capacity = new_cap;
	}
...
static void grid_line_ensure_cells(struct ypaint_canvas_grid_line *line, uint32_t min_cells)
{
	...
	line->cells = realloc(line->cells, new_cap * sizeof(struct ypaint_canvas_grid_cell));
...
static struct ypaint_canvas_grid_line *line_buffer_get(struct ypaint_canvas_line_buffer *buf,
						       uint32_t index)
{
	if (index >= buf->count)
		return NULL;
...
static void line_buffer_pop_front(struct ypaint_canvas_line_buffer *buf, uint32_t count)
{
...
void ypaint_canvas_set_max_prims_per_cell(struct ypaint_canvas *canvas, uint32_t max)
{
	if (canvas)
...
void ypaint_canvas_set_cursor(struct ypaint_canvas *canvas, uint16_t col, uint16_t row)
{
	if (canvas) {
				 ...
				 float aabb_max_x, float aabb_max_y)
{
	...
	int32_t local_min_row = (int32_t)floorf((aabb_min_y - base_y) / canvas->cell_size_y);
	int32_t local_max_row = (int32_t)floorf((aabb_max_y - base_y) / canvas->cell_size_y);
	if (local_min_row < 0)
		local_min_row = 0;
	if (local_max_row < 0)
		local_max_row = 0;
...
static void ensure_grid_staging(struct ypaint_canvas *canvas, uint32_t min_size)
{
	if (min_size <= canvas->grid_staging_capacity)
		return;

	uint32_t new_cap = canvas->grid_staging_capacity == 0 ?
			   INITIAL_STAGING_CAPACITY : canvas->grid_staging_capacity;
	while (new_cap < min_size)
		new_cap *= 2;

	canvas->grid_staging = realloc(canvas->grid_staging, new_cap * sizeof(uint32_t));
					    ...
					    void *user_data)
{
	...
	if (canvas->cell_size_x > 0.0f && canvas->scene_max_x > canvas->scene_min_x)
		grid_w = (uint32_t)ceilf((canvas->scene_max_x - canvas->scene_min_x) / canvas->cell_size_x);
	...
	// Extend for glyphs
	if (glyph_count > 0 && bounds_func && canvas->cell_size_x > 0 && canvas->cell_size_y > 0) {
		for (uint32_t gi = 0; gi < glyph_count; gi++) {
			...
			uint32_t max_cell_x = (uint32_t)fmaxf(0.0f,
					floorf((g_max_x - canvas->scene_min_x) / canvas->cell_size_x)) + 1;
			uint32_t max_cell_y = (uint32_t)fmaxf(0.0f,
					floorf((g_max_y - canvas->scene_min_y) / canvas->cell_size_y)) + 1;
	...
	if (glyph_count > 0 && bounds_func) {
		...
		for (uint32_t gi = 0; gi < glyph_count; gi++) {
			...
			int32_t c_min_x = (int32_t)floorf((g_min_x - canvas->scene_min_x) / canvas->cell_size_x);
			int32_t c_min_y = (int32_t)floorf((g_min_y - canvas->scene_min_y) / canvas->cell_size_y);
			int32_t c_max_x = (int32_t)floorf((g_max_x - canvas->scene_min_x) / canvas->cell_size_x);
			int32_t c_max_y = (int32_t)floorf((g_max_y - canvas->scene_min_y) / canvas->cell_size_y);
			...
			for (int32_t cy = c_min_y; cy <= c_max_y; cy++) {
				for (int32_t cx = c_min_x; cx <= c_max_x; cx++) {
					...
					cell_glyphs[cell_idx] = realloc(cell_glyphs[cell_idx],
	...
	for (uint32_t y = 0; y < grid_h; y++) {
		...
		struct ypaint_canvas_grid_line *line = has_line ?
				line_buffer_get(&canvas->lines, y) : NULL;
...
static void ensure_prim_staging(struct ypaint_canvas *canvas, uint32_t min_size)
{
	if (min_size <= canvas->prim_staging_capacity)
		return;

	uint32_t new_cap = canvas->prim_staging_capacity == 0 ?
			   INITIAL_STAGING_CAPACITY : canvas->prim_staging_capacity;
	while (new_cap < min_size)
		new_cap *= 2;
