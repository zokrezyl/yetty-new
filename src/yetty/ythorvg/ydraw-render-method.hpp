...
/*
 * YDrawRenderMethod — ThorVG RenderMethod that emits ypaint SDF primitives
 * into a yetty_ypaint_core_buffer.
 *
 * Ported from yetty-poc/src/yetty/ythorvg/ydraw-render-method.h with the
 * YDrawBuffer dependency replaced by yetty_ypaint_core_buffer + ysdf
...
public:
    ...
    explicit YDrawRenderMethod(yetty_ypaint_core_buffer* buffer);
    ...
    // Rendering - emits ypaint primitives
...
private:
    ...
    // Emit a mesh polygon: fan triangulation for fill + edges as segments for stroke.
    void emitMeshPolygon(uint32_t vertexCount, const float* vertices,
                         uint32_t fillColor, uint32_t strokeColor, float strokeWidth);
    void emitMeshPolygonGroup(uint32_t totalVertexCount,
    ...
    yetty_ypaint_core_buffer* _buffer = nullptr;  // borrowed
    uint32_t _width = 0;
    uint32_t _height = 0;
