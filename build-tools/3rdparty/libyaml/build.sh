#!/usr/bin/env bash
# libyaml 3rdparty wrapper: picks the right nix shell for $TARGET_PLATFORM
# and re-execs into _build.sh inside it. Windows uses MSYS2 CLANG64
# (same pattern as qemu / libuv).
#
# Required env:
...
cd "$(dirname "$0")/../../.."
exec nix develop ".#$SHELL_NAME" --command bash build-tools/3rdparty/libyaml/_build.sh "$@"
